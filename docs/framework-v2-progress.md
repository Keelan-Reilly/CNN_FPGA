...
- Extended the direct MAC-array slice from a calibrated baseline-only foothold into a two-scale, three-way directly measured bridge: 4x4 and 8x4 baseline, `shared_lut_saving`, and `shared_dsp_reducing` on the isolated direct top.
...
- Refined the taxonomy so framework `shared` outputs refer to the modelled shared family, while the direct 4x4 and 8x4 shared slices are tracked as separate implementation-specific measured observations.
